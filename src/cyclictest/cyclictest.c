...
static void *timerthread(void *param)
{
	...
	stat->threadstarted++;
    uint64_t diff_prev = 999999;
	while (!shutdown) {

		uint64_t diff, tmp, jitter;
		...
		/* The periodic jitter is a signed integer actually: J_per = T_per - T_0*/
        /* The difference of lantencis is equal to the difference of periods. */
		
        // jitter = | L1-L0 | 
		if (diff > diff_prev)
			jitter = diff - diff_prev;
		else 
			jitter = diff_prev - diff;
        ...
        //************************   top
        if (jitter < stat->min_j)
			stat->min_j = jitter;
		if (jitter > stat->max_j) {
			stat->max_j = jitter;
			if (refresh_on_max)
				pthread_cond_signal(&refresh_on_max_cond);
		}
        stat->avg_j += (double) jitter;
        //************************   bottom
...
static void print_hist(struct thread_param *par[], int nthreads)
{
	...
	fprintf(fd, "\n");
	*/
	fprintf(fd, "# Min Jitters:");
	for (j = 0; j < nthreads; j++)
		fprintf(fd, " %05lu", par[j]->stats->min_j);
	fprintf(fd, "\n");
	fprintf(fd, "# Avg Jitters:");
	for (j = 0; j < nthreads; j++)
		fprintf(fd, " %05lu", par[j]->stats->cycles ?
		       (long)(par[j]->stats->avg_j/par[j]->stats->cycles) : 0);
	fprintf(fd, "\n");
		...
		if (quiet != 1) {
		   ...
		   // 输出到终端
			if (use_nsecs)
				fmt = "T:%2d (%5d) P:%2d I:%ld C:%7lu "
				        "Min:%7ld Act:%8ld Avg:%8ld Max:%8ld "
                        "Min_j:%7ld Act_j:%8ld Avg_j:%8ld Max_j:%8ld";
			else
				fmt = "T:%2d (%5d) P:%2d I:%ld C:%7lu "
				        "Min:%7ld Act:%5ld Avg:%5ld Max:%8ld "
                        "Min_j:%7ld Act_j:%5ld Avg_j:%5ld Max_j:%8ld";

			fprintf(fp, fmt, index, stat->tid, par->prio,
				par->interval, stat->cycles, 
				stat->min,stat->act, stat->cycles ?
				(long)(stat->avg/stat->cycles) : 0, stat->max,
                stat->min_j,stat->act_j, stat->cycles ?
				(long)(stat->avg_j/stat->cycles) : 0, stat->max_j);
            //*********************************   bottom
